...
void main() async {
  ...
  Future<AtClient> initializeOAuthAtClient() async {
    ...
    await AtClientManager.getInstance()
        .setCurrentAtSign('@scorpiopersonal', 'atbuzz', atClientPreference);
  ...
  Future<void> handlePostRequest(HttpRequest request) async {
    ...
    if (request.uri.path == '/@sign') {
      ...
      if (result == true) {
        ...
        healthInfo = await authenticationService.getHealthInfoSharedBy();
        print(personalInfo.toString());
        print(contactInfo.toString());
        print(healthInfo.toString());
      }

      // Send custom response
      final message = {
        "personal_info": personalInfo.toString(),
        "contact_info": contactInfo.toString(),
        "health_info": healthInfo.toString()
      };
      String str = json.encode(message);
      response.write(str);
      response.close();
